...
# 가능하면 anon 키 사용, 꼭 필요할 때만 service key 사용 (절대 코드에 직접 적지 말 것)
...
# 줄별 스타일(최대 5줄)
for i in range(5):
    st.session_state.setdefault(f"title_align_label_{i}", "가운데")
...
preview_title_lines = []
for i in range(5):
    line = title_lines[i].strip()
    preview_title_lines.append(line if line else sample_defaults[i])
...
for i in range(5):
    ...
    title_aligns.append(align_label_to_value.get(align_label, "center"))
